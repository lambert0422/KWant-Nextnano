...
class Kwant_SSeS():
    ...
    def DefOutputMap(self):
        ...
        if self.SN == 'SN':
            ...
            self.Defect_Map = np.zeros((self.L+1 , 2 * self.WSC + self.W-2))
            self.Potential_Map = np.zeros((self.L+1 , 2 * self.WSC + self.W-2))
            self.Delta_abs_Map = np.zeros((self.L+1 , 2 * self.WSC + self.W-2))
            self.Delta_phase_Map = np.zeros((self.L+1 , 2 * self.WSC + self.W-2))
            self.Onsite_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
            self.gn_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
            self.MU_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
            self.alpha_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
            self.beta_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
            self.Vbias_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
            self.Tunnel_Map = np.zeros((self.L+1, 2 * self.WSC + self.W-2))
    ...
    def Run_sweep(self):
        ...
        def Delta_0_dis(x, y):
            ...
            self.Delta_abs_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = np.abs(DELTA)
            # self.Delta_abs_Map[int(x), int(y) + self.WSC] = np.angle(DELTA)

            self.Delta_phase_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = np.angle(DELTA)
        ...
        def EZ_x_dis(x, y):
            ...
            self.gn_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = np.real(g_muB)
        ...
        def alpha_dis(x, y):
            ...
            # alpha = self.alpha
            self.alpha_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = abs(alpha)
            return alpha
        ...
        def beta_dis(x, y):
            ...
            # beta =  self.beta
            self.beta_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = beta
            return beta
        def mu_dis(x, y):
            if self.Surface2DEG:
                Square = 1
            ...
            self.MU_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = MU
            return MU
        ...
        def VGate_dis(x, y):
            ...
            if self.DavidPot:
                try:
                    VGate = self.u_sl[int(x)+self.L_Side, int(y)+self.WSC -1]
                ...
                Y = (y) * self.W_r *self.VGate_y_f / self.W
            ...
            self.Potential_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = VGate
        ...
        def V_dis(x, y):
            ...
            self.Vbias_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = np.real(V)
            return V

        def TunnelBarrier_dis(x, y):

            TunnelBarrier = self.GammaTunnel * self.Tunnel_region(x,y)
            self.Tunnel_Map[int(x)+self.L_Side, int(y) + self.WSC -1] = np.real(TunnelBarrier)
